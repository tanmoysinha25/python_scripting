...
ko = 0.42
f = 0.60
nqh = [49]     # Bearing capacity factor can be obtained from a graph. So need to input it manually.
Tu = 0
...
for item in res:
    ...
    for value in Tu_Pu:
        ...
        Max_iterations=11      #Need to select based on the parameters combinations

        for item in range (q,Max_iterations):
            ...
            mdb.models['Model-%d' %(q)].Part(dimensionality=TWO_D_PLANAR, name='Part-%d' %(q), type=
                DEFORMABLE_BODY)
            mdb.models['Model-%d' %(q)].parts['Part-%d' %(q)].BaseWire(sketch=
                mdb.models['Model-%d' %(q)].sketches['__profile__'])
            del mdb.models['Model-%d' %(q)].sketches['__profile__']
            mdb.models['Model-%d' %(q)].PipeProfile(formulation=THICK_WALL, name='Profile-%d' %(q), r=
            ...
            mdb.models['Model-%d' %(q)].Material(name='Material-%d' %(q))
            mdb.models['Model-%d' %(q)].materials['Material-%d' %(q)].Density(table=((940.0, ), ))  #Density of MDPE pipe = 940 kg/m3
            mdb.models['Model-%d' %(q)].materials['Material-%d' %(q)].Elastic(table=((566278000.0,  #Elastic & Plastic property at strain rate 10^-4/s
                0.45), ))
            mdb.models['Model-%d' %(q)].materials['Material-%d' %(q)].Plastic(table=((1700000.0, 0.0), 
            ...
            mdb.models['Model-%d' %(q)].BeamSection(consistentMassMatrix=False, integration=
                DURING_ANALYSIS, material='Material-%d' %(q), name='Section-%d' %(q), poissonRatio=0.0, 
                profile='Profile-%d' %(q), temperatureVar=LINEAR)
            mdb.models['Model-%d' %(q)].parts['Part-%d' %(q)].SectionAssignment(offset=0.0, 
                offsetField='', offsetType=MIDDLE_SURFACE, region=Region(
                edges=mdb.models['Model-%d' %(q)].parts['Part-%d' %(q)].edges.getSequenceFromMask(
                mask=('[#1 ]', ), )), sectionName='Section-%d' %(q), thicknessAssignment=
                FROM_SECTION)
            mdb.models['Model-%d' %(q)].parts['Part-%d' %(q)].assignBeamSectionOrientation(method=
            ...
            mdb.models['Model-%d' %(q)].rootAssembly.Instance(dependent=ON, name='Part-1-%d' %(q), 
                part=mdb.models['Model-%d' %(q)].parts['Part-%d' %(q)])
            ...
            mdb.models['Model-%d' %(q)].parts['Part-%d' %(q)].seedPart(deviationFactor=0.1,    #Mesh size 5 mm chosen
                minSizeFactor=0.1, size=0.005)
            mdb.models['Model-%d' %(q)].parts['Part-%d' %(q)].generateMesh()
            mdb.models['Model-%d' %(q)].parts['Part-%d' %(q)].setElementType(elemTypes=(ElemType(
                elemCode=PIPE21, elemLibrary=STANDARD), ), regions=(                         # Element type: PIPE21
                mdb.models['Model-%d' %(q)].parts['Part-%d' %(q)].edges.getSequenceFromMask(('[#1 ]', 
                ), ), ))
            ...
            mdb.models['Model-%d' %(q)].rootAssembly.Set(name='Displacement', nodes=
                mdb.models['Model-%d' %(q)].rootAssembly.instances['Part-1-%d' %(q)].nodes.getSequenceFromMask(
                mask=('[#0:5 #7c0000 ]', ), ))
            ...
            mdb.models['Model-%d' %(q)].keywordBlock.replace(29, 
                '\n*Boundary, amplitude=Amp-1\nDisplacement, 2, 2, -0.12\n'+'Part-1-%d' %(q)+'.psi_far_end_Set_Soil,1,1\n'+'Part-1-%d' %(q)+'.psi_far_end_Set_Soil,2,2\n'+'Part-1-%d' %(q)+'.psi_far_end_Set_Soil,3,3')  # Boundary condition of PSI elements.
            ...
            session.xyDataListFromField(odb=o1, outputPosition=NODAL, variable=(('RF', 
                NODAL, ((COMPONENT, 'RF2'), )), ), nodeLabels=(('PART-1-%d' %(q), ('179:183', )),   #The physical distance between node 179 to 183 is 25 mm which is equal to the pulling grip width.
                ))
            xy1 = session.xyDataObjects['RF:RF2 PI: '+'PART-1-%d' %(q)+' N: 179']
            xy2 = session.xyDataObjects['RF:RF2 PI: '+'PART-1-%d' %(q)+' N: 180']
            xy3 = session.xyDataObjects['RF:RF2 PI: '+'PART-1-%d' %(q)+' N: 181']
            xy4 = session.xyDataObjects['RF:RF2 PI: '+'PART-1-%d' %(q)+' N: 182']
            xy5 = session.xyDataObjects['RF:RF2 PI: '+'PART-1-%d' %(q)+' N: 183']
            xy6 = sum((xy1, xy2, xy3, xy4, xy5))
            xy6.setValues(
                sourceDescription='sum ( ( "RF:RF2 PI: '+'PART-1-%d' %(q)+' N: 179", "RF:RF2 PI: '+'PART-1-%d' %(q)+' N: 180", "RF:RF2 PI: '+'PART-1-%d' %(q)+' N: 181", "RF:RF2 PI: '+'PART-1-%d' %(q)+' N: 182", "RF:RF2 PI: '+'PART-1-%d' %(q)+' N: 183" ) )')
            tmpName = xy6.name
            ...
            session.xyDataListFromField(odb=o1, outputPosition=NODAL, variable=(('LE', 
                INTEGRATION_POINT, ((COMPONENT, 'LE11'), )), ), nodeLabels=(('PART-1-%d' %(q), (
                '175', )), ))
            x2 = session.xyDataObjects['LE:LE11 (Avg: 75%) SP:2 PI: '+'PART-1-%d' %(q)+' N: 175']
            x3 = session.xyDataObjects['LE:LE11 (Avg: 75%) SP:14 PI: '+'PART-1-%d' %(q)+' N: 175']
